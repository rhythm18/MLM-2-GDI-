...
$id=ReturnAnyValue($conn,$sql);
  ...
  $sql="select * from users where pay_status='1'";
  $rs=mysqli_query($conn,$sql);

$strUser="";

$sql="select daily_income from income_setting where id=1";
$daily_income=ReturnAnyValue($conn,$sql);
...
if($daily_income>0)
{
      ...
      while($row=mysqli_fetch_array($rs))
          {
              $sql="select COUNT(*) as team_cnt from users where user_id>'$id'";
              $teamCnt=ReturnAnyValue($conn,$sql);


              $sql="select COUNT(sponsor_id) from users where sponsor_id='$id'";
              $sponsorCnt=ReturnAnyValue($conn,$sql);

              if($sponsorCnt>0 && $teamCnt>=$levelOneCount) // user will get daily income if he has a minimium of one sponsor
              {
                $dt=date('Y-m-d');
                $sql="select COUNT(*) from daily_income where inc_date='$dt' and user_id='$id'";
                //echo $sql;
              ...
              //  echo $cnt;
                ...
                if($cnt==0)
                  
                  {
                    ...
                    $sql="select joining_fee from income_setting where id=1";
                    $join_fee=ReturnAnyValue($conn,$sql);

                    $inc_amt=$join_fee*$daily_income/100;
                    $inc_date=date('Y-m-d');

                    $sql="insert into daily_income(user_id,inc_amt,inc_date) values('$id','$inc_amt','$inc_date')";
                    echo $sql;

                      if(mysqli_query($conn,$sql))
                      {
                        $strUser=$strUser."<br>".$id;
